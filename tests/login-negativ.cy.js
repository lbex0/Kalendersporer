describe("Login flow - negativ", () =>
{
  it("innlogging skal feile og det skal ikke navigeres til calendar siden", () =>
    ...
    cy.get("#content-kalendersporer---login--usernameInput-inner").should('be.visible').type("nora.kristnsen");
    ...
    cy.contains(/invalid|wrong|error|feil|could not log in|incorrect/i, { timeout: 3000 }).should('exist');

    // Se om vi forblir på login, feltene skal fortsatt være tilgjengelige
    cy.get("#content-kalendersporer---login--usernameInput-inner").should('be.visible');
