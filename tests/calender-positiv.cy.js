describe("Positiv test – finne Tobias sin bursdag og logge ut", () => 
{
  ...
  it("skal søke etter Tobias, åpne bursdagen hans og logge ut", () => {
    ...
    cy.get('#content-kalendersporer---calender--PC1-Header-NavToolbar-NextBtn-img').should('exist').closest('button').click();
    cy.wait(200);
    cy.get('#content-kalendersporer---calender--PC1-Header-NavToolbar-PrevBtn-img').should('exist').closest('button').click();
    ...
    // Gå tilbake til månedvisning for å se Tobias sin bursdag i juni
    cy.get('#content-kalendersporer---calender--PC1-Header-NavToolbar-PrevBtn-img').should('exist').closest('button').click();
    ...
    cy.get('#content-kalendersporer---login--passwordInput-inner').should('be.visible');
  });
});